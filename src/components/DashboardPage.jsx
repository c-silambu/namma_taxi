...
export default function DashboardPage({ session, setPage, showToast }) {
  ...
  useEffect(() => {
    if (!session?.admin_logged_in) { setPage("admin-login"); return; }
    loadData();
  }, []);
  ...
  const resetService = async (carId) => {
    if (!confirm("Service finished? Reset service warning for this car?")) return;
    const res = await api.post(`/api/cars/${carId}/service-reset`, {});
    if (res.success) {
      showToast("Service warning reset", "success");
      loadData();
    } else showToast(res.message || "Failed", "error");
  };
        ...
        <div style={{ padding: "24px 20px" }}>
          ...
          {tab === "overview" && (
            <>
              ...
              {data.cars.some((car) => car.service_warning) && (
                <div style={{ marginTop: 18, padding: 16, borderRadius: 14, border: "1px solid rgba(239,68,68,.35)", background: "rgba(239,68,68,.10)" }}>
                  <h3 style={{ color: "#ef4444", marginBottom: 10, fontSize: 16 }}>⚠️ Service Notification</h3>
                  {data.cars.filter((car) => car.service_warning).map((car) => (
                    <div key={car.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", color: text, fontSize: 13, marginBottom: 8 }}>
                      <span>{car.make} {car.model} ({car.license_plate}) — 12,000 km service reached</span>
                      <button style={styles.delBtn} onClick={() => resetService(car.id)}>Service Reset</button>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                  <AdminTable
                    ...
                    rows={data.trips.slice(-5).reverse().map((t) => [
                      ...
                      t.vehicle_category ? t.vehicle_category.replace(/_/g, " ").toUpperCase() : `${t.car_make || ""} ${t.car_model || ""}`,
            ...
            <>
              <div className="dash-tab-header">
                ...
                <button style={styles.heroCta} className="dash-add-btn" onClick={() => setCarForm({
                  ...
                  emi_date: "", service_date: "", current_km: 0, last_service_km: 0, service_interval_km: 12000
                })}>+ Add Car</button>
              </div>
              {carForm && (
                ...
                {data.cars.map((c) => (
                  <div key={c.id} style={{ background: styles.darkCard, border: `1px solid ${border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px", fontSize: 13, color: sub }}>
                      ...
                      <span>🛣️ {Number(c.current_km || 0).toLocaleString()} km</span>
                      {c.service_warning && <span style={{ gridColumn: "1/-1", color: "#ef4444", fontWeight: 700 }}>⚠️ Service due: 12,000 km reached</span>}
                      {c.service_warning && <button style={{ ...styles.delBtn, gridColumn: "1/-1" }} onClick={() => resetService(c.id)}>Service Reset</button>}
              ...
              <div className="dash-desktop-table" style={{ overflowX: "auto" }}>
                <AdminTable
                  cols={["Make", "Model", "Year", "Plate", "KM", "Service", "Status", "Actions"]}
                  rows={data.cars.map((c) => [
                    c.make, c.model, c.year, c.license_plate,
                    `${Number(c.current_km || 0).toLocaleString()} km`,
                    c.service_warning ? <button key={`service-${c.id}`} style={styles.delBtn} onClick={() => resetService(c.id)}>⚠️ Reset</button> : `${Number(c.service_due_km || 12000).toLocaleString()} km left`,
                    <span key={c.id} style={{
                      background: c.status === "available" ? "#22c55e20" : "#ef444420",
                      color: c.status === "available" ? "#22c55e" : "#ef4444",
                      padding: "2px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600
          ...
          {tab === "trips" && (
            <>
                ...
                {data.trips.map((t) => (
                  <div key={t.id} style={{ background: styles.darkCard, border: `1px solid ${border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px", fontSize: 13, color: sub }}>
                      ...
                      <span>🚗 {(t.vehicle_category || "mini").replace(/_/g, " ").toUpperCase()}</span>
                      <span>🕒 {t.drop_time || "—"}</span>
                      <span style={{ gridColumn: "1/-1" }}>📍 {t.pickup_location} → {t.drop_location}</span>
                      <span style={{ gridColumn: "1/-1" }}>Trip: {(t.trip_type || "one_way").replace(/_/g, " ")}</span>
                      <span style={{ color: gold, fontWeight: 700 }}>₹{t.total_fare?.toFixed(0)}</span>
                      <span>{t.status || "Pending"}</span>
                      {t.cancel_reason && <span style={{ gridColumn: "1/-1", color: "#ef4444" }}>Reason: {t.cancelled_by} - {t.cancel_reason}</span>}
                    </div>
                  </div>
                ))}
              </div>
              <div className="dash-desktop-table" style={{ overflowX: "auto" }}>
                <AdminTable
                  cols={["#", "Customer", "Customer Phone", "Driver", "Driver Phone", "Vehicle", "Trip Type", "Pickup", "Drop", "Date", "Drop Time", "Fare", "Status", "Cancel Reason", "OTP"]}
                  rows={data.trips.map((t) => [
                    t.id, t.customer_name, t.customer_phone || "—", t.driver_name || "—", t.driver_phone || "—",
                    t.vehicle_category ? t.vehicle_category.replace(/_/g, " ").toUpperCase() : `${t.car_make || ""} ${t.car_model || ""}`,
                    (t.trip_type || "one_way").replace(/_/g, " "),
                    t.pickup_location, t.drop_location, t.trip_date, t.drop_time || "—",
                    `₹${t.total_fare?.toFixed(0)}`,
                    t.status || "Pending", t.cancel_reason ? `${t.cancelled_by || ""}: ${t.cancel_reason}` : "—", t.otp || "—",
                  ])}
                />
              </div>
            </>
